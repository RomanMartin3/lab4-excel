...
const Modal: React.FC<ModalProps> = ({ show, handleClose, instrumento }) => {
  const { carrito, agregarAlCarrito, modificarCantidad, eliminarItem } =
    useCart(); // Usamos el hook
  ...
  const generarPDF = async () => {
    try {
      const response = await fetch(
        `http://localhost:8080/api/instrumentos/generar-pdf/${instrumento.id}`,
        {
          method: "GET",
          headers: {
      ...
      link.href = url;
      link.download = `instrumento_${instrumento.id}.pdf`;
